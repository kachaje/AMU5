...
function askDosage(){
    ...
    qtblcell1_1.innerHTML = "Select Dosage";
    ...
    var f = 0;
    for(var dose in dosages[current_concept_id]){
...
function processDrug(concept_id){
    ...
    drug += "; Dos.: <i>" + __$("editDosage").value + "</i>; Freq.: <i>" + __$("editFrequency").value + 
...
function removeGenerics(){    
    for(var i in __$("ulDoses").children){
        ...
        var generic = formulation[0].split(":")[1].trim();
        var dosage = formulation[1].split(":")[1].trim();
        var frequency = formulation[2].split(":")[1].trim();
        var duration = formulation[3].split(":")[1].trim();
        var prn = formulation[4].split(":")[1].trim();
        var strength = dosages[concept_id][dosage][0];
        var units = dosages[concept_id][dosage][1];
        ...
        var dosageFld = document.createElement("input");
        dosageFld.type = "hidden";
        dosageFld.name = "prescriptions[][dosage]";
        dosageFld.value = dosage;

        document.forms[0].appendChild(dosageFld);

        var strengthFld = document.createElement("input");
        strengthFld.type = "hidden";
        strengthFld.name = "prescriptions[][strength]";
        strengthFld.value = strength;

        document.forms[0].appendChild(strengthFld);
                
        var unitsFld = document.createElement("input");
        unitsFld.type = "hidden";
        unitsFld.name = "prescriptions[][units]";
        unitsFld.value = units;

        document.forms[0].appendChild(unitsFld);
